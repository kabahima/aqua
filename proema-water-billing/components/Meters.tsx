  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-8">
        <h1 className="text-3xl lg:text-4xl font-bold text-dark">Meters</h1>
        <button
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
        ...
        {meters.length > 0 ? (
          <table className="w-full text-left responsive-table">
            ...
            <tbody>
              {meters.map(meter => (
                <tr key={meter.id} className="md:border-b md:hover:bg-gray-50">
                  <td className="text-gray-700 md:p-4" data-label="Meter ID">{meter.id}</td>
                  <td className="text-gray-700 md:p-4" data-label="Customer">{getCustomerName(meter.customerId)}</td>
                  <td className="text-gray-700 md:p-4" data-label="Serial Number">{meter.serialNumber}</td>
          ...
          <p className="text-center py-6 text-gray-500">No meters found.</p>
        )}
      </div>
    </div>
  );
