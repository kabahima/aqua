...
import { Customer, Meter, Reading, Bill, Payment } from '../types';
import { UsersIcon, MeterIcon, ReadingIcon, CashIcon, BillingIcon } from './icons';
...
const Dashboard: React.FC<DashboardProps> = ({ customers, meters, readings, bills, payments }) => {
  ...
  const totalPayments = payments.reduce((acc, p) => acc + p.amountPaid, 0);
  const totalOutstanding = bills
    .filter(b => b.status !== 'Paid')
    .reduce((acc, b) => acc + b.balanceDue, 0);

  return (
    <div>
      <h1 className="text-3xl lg:text-4xl font-bold text-dark mb-8">Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard icon={CashIcon} title="Payments Collected" value={`UGX ${Math.round(totalPayments).toLocaleString('en-US')}`} color="bg-indigo-500" />
        <StatCard icon={BillingIcon} title="Outstanding Balance" value={`UGX ${Math.round(totalOutstanding).toLocaleString('en-US')}`} color="bg-red-500" />
      </div>
